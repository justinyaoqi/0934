...
/*点击赞*/
function NoHelp(tid) {
    $("#nohelp_"+tid).html(parseInt($("#nohelp_"+tid).html(),10)+1);
    //alert($();
}
/*点击没有帮助---tid--问题id*/
...
function taoalbum(topicid) {
    $.post(siteUrl + 'index.php?app=group&ac=album&ts=topic', {
        'topicid': topicid
    }, function(rs) {
        if (rs == 0) {
            art.dialog({
...
function loadTopic(userid, page) {
    ...
    $.get(siteUrl + 'index.php?app=group&ac=ajax&ts=topic', {
        'userid': userid,
        'page': page
    }, function(rs) {
        if (rs == '') {
...
function topicAudit(topicid) {
    $.post(siteUrl + 'index.php?app=group&ac=ajax&ts=topicaudit', {
        'topicid': topicid
    }, function(rs) {
        if (rs == 0) {
            art.dialog({
...
function setAdmin(groupid, userid, token) {
    ...
    $.post(siteUrl + 'index.php?app=group&ac=user&ts=manager', {
        'groupid': groupid,
        'userid': userid,
        'token': token
    }, function(rs) {
...
function kickedGroup(groupid, userid) {
    $.post(siteUrl + 'index.php?app=group&ac=kicked', {
        'groupid': groupid,
        'userid': userid
    }, function(rs) {
        if (rs == '0') {
